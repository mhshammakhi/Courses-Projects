...
for k=1:numClasses
    ...
    models{k} = fitcsvm(TrainingSet,G1vAll,'Standardize',true,'KernelFunction','RBF','KernelScale','auto');
end

%classify test cases

    for k=1:numClasses
        [a,b{k},c]=predict(models{k},TestSet); 
    end
    score=cell2mat(b);score=score(:,1:2:end);
    score_comp=repmat(min(score,[],2),1,numClasses); 
    
    [z,w]=find(score'==score_comp');
%    [z,w]=find(score'==ones(size(score')));
    temp=zeros(size(TestSet,1),1);
    temp(w)=z;
    z=temp;
end
