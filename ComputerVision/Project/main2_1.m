...
% [Length,height,width,Movie]=Import_mov([path,temp]);
...
for t=1:Movie.Length-1
    ...
    for j=1:length(Prev_Frame.Corner_HOG)
        for i=1:length(Crnt_Frame.Corner_HOG)
            err(i,j)=sum(((Crnt_Frame.Corner_HOG{i}-Prev_Frame.Corner_HOG{j}).^2)./(Crnt_Frame.Corner_HOG{i}+Prev_Frame.Corner_HOG{j}+eps));
        end
    end
    if ~isempty(err)
        if(t==33)
        ...
        [feature_final(1),feature_final(2)]=find(err==min(min(err)));min(min(err))
        if min(min(err))<85.15
            temp=Crnt_Frame.corners(feature_final(1),:);A=[temp(1);temp(2);1];
            temp=Prev_Frame.corners(feature_final(2),:);B=[temp(1);temp(2);1];
           
            
            subplot(2,2,1)
            imshow(Crnt_Frame.T_frame);hold all;
            plot(Crnt_Frame.Corner_Struct(feature_final(1)))
            hold off;
            subplot(2,2,2)
            imshow(Prev_Frame.T_frame);hold all;
            plot(Prev_Frame.Corner_Struct(feature_final(2)))
            hold off;
...
%         else
            ...
            for j=1:length(Prev_Frame.Corner_HOG)
                for i=1:length(Crnt_Frame.Corner_HOG)
                    err(i,j)=sum((Crnt_Frame.Corner_HOG{i}+0.001)./(Prev_Frame.Corner_HOG{j}+0.001).*(Crnt_Frame.Corner_HOG{i})+...
                        (Prev_Frame.Corner_HOG{j}+0.001)./(Crnt_Frame.Corner_HOG{i}+0.001).*(Prev_Frame.Corner_HOG{j}));
                end
            end
